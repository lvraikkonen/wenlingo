...
Dev manual QA on 2026-06-11 surfaced three follow-up items that are accepted as
the V0.5b hotfix scope, rather than changes to the original sentence-training
vertical slice:

- Existing Alpha accounts need a no-invite login path on `/alpha/start` so a
  parent can open a new browser/device, request a Magic Code by email, and land
  on `/parent/children` when the account is already linked to a family.
- QA and ops need a manual database-level Alpha account cleanup script for Dev
  test data. The script must default to dry-run, reject production, reuse the
  existing tested delete graph, and require `--allow-real-email` before deleting
  real-domain test inboxes such as QQ or Hotmail.
- Environment variables need a canonical checklist plus README entry so Dev and
  Railway configuration does not miss critical switches such as
  `LLM_DAILY_LIMIT_ENABLED`, Resend settings, CORS/auth origins, or the web
...
SENTENCE_CHALLENGE_SYSTEM_PROMPT = (
    "你是一名小学三到六年级中文句子训练教练。你必须只输出 JSON。"
...
SENTENCE_CHALLENGE_GENERATION_PROMPT = register_prompt(
    PromptSpec(
        ...
        response_contract=(
            ...
            "五年级基础, 五年级进阶, 六年级基础, 六年级进阶, matching grade_label; "
            "grade_label: one of 三年级, 四年级, 五年级, 六年级, matching the request payload."
        ),
    )
)
...
from app.services.sentence_challenges import (
    ...
    SUPPORTED_SENTENCE_CHALLENGE_GRADE_LABELS,
...
def test_fallback_challenge_returns_valid_supported_type_and_grade():
    challenge = fallback_challenge("action_expression", grade_label="五年级")
...
def test_sentence_challenge_supported_grades_are_alpha_child_grades():
    assert SUPPORTED_SENTENCE_CHALLENGE_GRADE_LABELS == (
        "三年级",
        "四年级",
        "五年级",
        "六年级",
...
def test_sentence_challenge_contract_accepts_supported_grade_labels():
    for grade_label, difficulty_label in [
        ("三年级", "三年级基础"),
        ("四年级", "四年级进阶"),
        ("五年级", "五年级基础"),
        ("六年级", "六年级进阶"),
...
from app.services.llm_contracts import ChallengeGrade, SentenceChallenge, SentenceChallengeFeedback


SUPPORTED_SENTENCE_CHALLENGE_GRADE_LABELS: tuple[ChallengeGrade, ...] = (
    "三年级",
    "四年级",
    "五年级",
    "六年级",
...
def challenge_type_spec(target_skill: str) -> ChallengeTypeSpec:
    try:
        ...
        raise ValueError(f"unsupported sentence challenge target_skill: {target_skill}") from exc


def validate_sentence_challenge_grade_label(grade_label: str) -> ChallengeGrade:
...
def fallback_challenge(target_skill: str, grade_label: str) -> SentenceChallenge:
    ...
    supported_grade = validate_sentence_challenge_grade_label(grade_label)
    return SentenceChallenge(
        ...
        focus=spec.focus,
        difficulty_label=f"{supported_grade}基础",
        grade_label=supported_grade,
    )


def fallback_challenge_feedback(target_skill: str) -> SentenceChallengeFeedback:
    spec = challenge_type_spec(target_skill)
    if spec.target_skill == "feeling":
...
In `apps/api/app/services/ai_tasks.py`, add a task-specific validation helper and wrappers. The helper should raise a validation-style `ValueError` when the model returns a supported-but-wrong target skill or grade label, so the existing retry/fallback logging path treats it like schema validation failure:

```python
def validate_generated_sentence_challenge(
    challenge: SentenceChallenge,
    ...
    if not challenge.difficulty_label.startswith(requested_grade_label):
        raise ValueError(
            "sentence challenge difficulty_label mismatch: "
            f"requested grade {requested_grade_label}, got {challenge.difficulty_label}"
        )
```
...
async def sentence_challenge_generation(
    ...
    return await run_validated_llm_task(
        ...
        prompt_version=prompt.version,
        validation_hook=lambda challenge: validate_generated_sentence_challenge(
            challenge,
            requested_target_skill=target_skill,
            requested_grade_label=grade_label,
        ),
...
Add tests in `apps/api/tests/test_ai_task_resilience.py` proving:

- `sentence_challenge_generation()` sends `target_skill` and the requested `grade_label` in the provider payload.
- Supported grade labels `三年级`, `四年级`, `五年级`, and `六年级` are accepted.
- Unsupported grade labels raise `ValueError` before provider calls.
- A model response whose supported `target_skill`, `grade_label`, or `difficulty_label` grade prefix does not match the request retries and then falls back to the requested target/grade with `validation_ok=False`.
- `sentence_challenge_feedback()` still logs only target skill and sentence lengths, not raw child writing.
...
```bash
git add apps/api/app/services/llm_contracts.py apps/api/app/services/sentence_challenges.py apps/api/app/services/ai_tasks.py apps/api/tests/test_sentence_challenge_service.py apps/api/tests/test_llm_contracts.py apps/api/tests/test_ai_task_resilience.py
...
def test_challenge_generation_persists_generated_training(session):
    ...
    assert payload["challenge"]["grade_label"] == student.grade_label
...
def test_challenge_generation_uses_student_grade_label(session):
    ...
    assert response.status_code == 201
    payload = response.json()
    assert payload["challenge"]["grade_label"] == "五年级"
    assert payload["challenge"]["difficulty_label"] == "五年级基础"
...
- Type consistency:
  ...
  - Sentence challenge grade labels support `三年级`, `四年级`, `五年级`, and `六年级`; API generation uses `StudentProfile.grade_label`, not client-provided grade.
