...
export function MaterialCardsStep({
  ...
  const orderedVisibleCards = visibleCards
    .slice()
  ...
  function moveCard(cardId: string, direction: -1 | 1) {
    const currentIndex = orderedVisibleCards.findIndex(
      (card) => card.id === cardId,
    );
    ...
    if (
      currentIndex < 0 ||
      targetIndex < 0 ||
      targetIndex >= orderedVisibleCards.length
    ) {
      return;
    }

    const reordered = [...orderedVisibleCards];
    ...
    onCardsChange(
      cards.map((card) =>
        orderById.has(card.id)
          ? { ...card, order: orderById.get(card.id)! }
          : card,
      ),
    );
  }
  ...
  function deleteCard(cardId: string) {
    onCardsChange(
      cards.map((card) =>
        card.id === cardId
          ? { ...card, deleted: true, child_edited: true }
          : card,
      ),
    );
  }
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <button
                className="rounded-lg border border-[var(--wen-border)] px-3 py-2 text-sm font-semibold disabled:opacity-50"
                type="button"
                disabled={index === orderedVisibleCards.length - 1}
                onClick={() => moveCard(card.id, 1)}
              >
                下移
              </button>
